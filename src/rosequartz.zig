...
const Server = struct
{
    ...
    fn handleKeybind(server: *Server, key: xkbc.Keysym) bool 
    {
        switch (key) {
            // kill compositor
...
const Keyboard = struct
{
    ...
    fn create(server: *Server, device: *wlr.InputDevice) !void 
    {
        ...
        const context = xkbc.Context.new(.no_flags) orelse return error.ContextFailed;
        defer context.unref();
        const keymap = xkbc.Keymap.newFromNames(context, null, .no_flags) orelse return error.KeymapFailed;
        defer keymap.unref();
